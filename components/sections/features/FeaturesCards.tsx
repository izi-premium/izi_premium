...
const FeaturesCards = () => {
  ...
  return (
    <div className="flex-center-col w-full min-w-full items-stretch gap-10 md:items-center md:justify-center xl:flex-row xl:items-stretch">
      {cards.map((card, index) => (
        <div
          key={index}
          className="border-primary-text-500 bg-elevated-surfaces-500 flex w-full flex-col items-start justify-between gap-6 rounded-t-[0.8rem] border border-solid px-4 pt-4 pb-0 md:max-w-[44rem] lg:gap-[clamp(2.4rem,1.25vw)] xl:max-w-[90rem] xl:items-stretch xl:border-b-0 2xl:gap-12"
        >
          <div className="flex-start-col flex-shrink-0 gap-2">
          ...
          <div className="border-primary-text-700 relative mt-auto flex h-[16rem] w-full min-w-full flex-shrink-0 items-end justify-end rounded-tl-[0.4rem] rounded-tr-[0.4rem] border border-b-0 border-solid bg-white md:h-[24rem] xl:h-[26rem]">
            <div className="relative h-[16rem] w-full md:h-[24rem]">
              <Image
                ...
                className="bg-bottom bg-no-repeat object-contain"
              />
            </div>
          </div>
        </div>
